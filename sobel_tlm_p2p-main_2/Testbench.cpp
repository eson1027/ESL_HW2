...
Testbench::Testbench(sc_module_name n)
  ...
  SC_THREAD(do_sobel); //多了initiator作為submodule
}
...
void Testbench::do_sobel() {
  int x, y, v, u=0;        // for loop counter
    ...
    for (x = -2; x != width + 2; ++x) {
      ...
      for (v = -yBound; v != yBound + adjustY; ++v) {   //-1, 0, 1
        //for (u = -xBound; u != xBound + adjustX; ++u) { //-1, 0, 1
          ...
          //sc_time delay(1 * CLOCK_PERIOD, SC_NS);
        //}
      }
      ...
      while(!done){
        ...
        if(output_num>0) done=true; //資料讀完成了
      }
      //wait(10 * CLOCK_PERIOD, SC_NS);
      //sc_time delay(10 * CLOCK_PERIOD, SC_NS);
      initiator.read_from_socket(SOBEL_FILTER_RESULT_ADDR, mask, data.uc, 4);
      total = data.sint; //像是total = i_result.read();
      //debug
